...
import { deleteThisCoverLetter } from "@/Services/coverLetterAPI";
...
function CoverLetterCard({ coverLetter, refreshData }) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    console.log("Delete Cover Letter with ID", coverLetter._id);
    try {
      const response = await deleteThisCoverLetter(coverLetter._id);
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-around p-4 bg-white rounded-b-lg shadow-md">
        <Button
          variant="ghost"
          onClick={() => navigate(`/dashboard/view-cover-letter/${coverLetter._id}`)}
          className="mx-2"
        >
        ...
        <Button
          variant="ghost"
          onClick={() => navigate(`/dashboard/edit-cover-letter/${coverLetter._id}`)}
          className="mx-2"
        >
